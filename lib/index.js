...
GLSL.prototype.compile = function compile (arg) {
	return this.stringify(this.parse(arg));
};
...
GLSL.prototype.transforms = {
	...
	binary: function (node) {
		...
		if (node.data === '[') {
			...
			// 	//test if it is nested access like a[2][3] -
			// 	if (leftNode.type === 'binary') {
			// 		var outerRight = this.stringify(leftNode.children[1]);
			// 		if (/[0-9]+/.test(outerRight)) {
			// 			var res = this.getComponent(leftNode.children[0], +outerRight);

			// 			//if possible to reach inner element - return it
			// 			if (Array.isArray(res)) {
			// 				return res[+right];
			// 			}

			// 			//otherwise - render by default
			// 		}
			// 	}

			// 	//normal access a[n]
			// 	var res = this.getComponent(leftNode, +right);
	...
	unary: function (node) {
		...
		if (node.data === '+') return this.stringify(node.children[0]);
...
GLSL.prototype.stringifyStdlib = function (includes) {
	...
	for (var meth in includes) {
		...
		if (this.replaceStdlib) {
			...
			//FIXME: atan fails here, as it may take 2 args
			if (Math[meth]) {
				result = `Math.${meth};`;
			} else {
				result = this.stdlib[meth].toString();
			}
		}
		methods.push(result);
	}
...
/**
 * Return call node, representing a type store.
 * Just a helper.
 */
GLSL.prototype.createCallNode = function (caller, args) {
	var ast = this.parse(`+${caller}(${args.join(',')});`);

	//eval ast to init components on the node
	this.stringify(ast);

	return ast.children[0].children[0].children[0].children[0];
};
