...
GLSL.prototype.transforms = {
	...
	binary: function (node) {
		...
		if (node.data === '[') {
			...
			if (/[0-9]+/.test(right)) {
				//test if it is nested access like a[2][3] - return nested components, if any
				...
				//mat returns array - that is an array of components of a vector
				if (Array.isArray(res)) {
					res = `[${res.join(', ')}]`;
				}
		...
		if (!this.primitives[typeA] || !this.primitives[typeB]) {
			...
			if ((!/mat/.test(typeA) && !/mat/.test(typeB)) && this.complexity(node.children[0]) + this.complexity(node.children[1]) < this.maxComplexity) {
...
GLSL.prototype.createTypeResult = function (str, compNodes, typeName) {
	...
	for (var i = 0; i < compNodes.length; i++) {
		...
		//if component is array - treat it as vector component (matrices)
		if (/vec/.test(typeName)) {
			var comp = this.getComponents(curr);
		}
	...
	if (this.complexity(compNodes) >= this.maxComplexity) {
		...
		var compStr = components.map(function (el) {
			if (Array.isArray(el)) {
				return `[${el.join(', ')}]`;
			} else {
				return el;
		...
		var result = new String(`[${compStr.join(', ')}]`);
	}
