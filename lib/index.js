...
GLSL.prototype.compile = function compile (arg) {
	var result = this.stringify(this.parse(arg));
...
GLSL.prototype.transforms = {
	...
	struct: function (node) {
		...
		this.structures[structName] = function (args) {
			...
			var fields = argsList.map(function (argName, i) {
				if (args[i]) {
					var initValue = this.stringify(args[i]);
				} else {
	...
	preprocessor: function (node) {
		...
		if (/^define/.test(macroName)) {
			...
			var rest = str.slice(restIdx + varName.length + macroArgs.length).trim();
			var fnArgs = [];
		...
		if (/^undef/.test(macroName)) {
			var varNameOnly = /[a-z0-9_]+/.exec(varName)[0];
	...
	binary: function (node) {
		...
		if (node.data === '[') {
			...
			if (leftNode.type === 'binary') {
				...
				var outerRight = this.stringify(leftNode.children[1]);
				...
				//if calc - do slice
					...
					return `${this.stringify(matNode)}[${outerRight} * ${vecSize} + ${right}]`;
				}
			}
	...
	unary: function (node) {
		...
		if (node.data === '+') {
			...
			//+x
			return this.stringify(node.children[0]);
		}
		return node.data + this.stringify(node.children[0]);
	},
	...
	call: function (node) {
		...
		//else treat as function/constructor call
			...
			//someFn()
				...
				return `${callName}(${args.map(this.stringify, this).join(', ')})`;
			}
		}
...
GLSL.prototype.getComponent = function (node, n) {
	...
	if (isNaN(idx)) {
		return `${this.stringify(node)}[${idx}]`;
	}
...
GLSL.prototype.getMatrixComponent = function (node, n) {
	...
	//calculated access should slice array
		...
		return `${this.stringify(node)}.slice(${n}*${size}, (${n} + 1) * ${size})`;
	}
}
