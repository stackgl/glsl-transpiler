	...
	binary: function (node) {
		...
		if (node.data === '[') {
			...
			if (/mat/.test(leftType)) {
				...
				if (!isNaN(offset)) {
					var components = this.getComponents(leftNode).slice(offset*size, (offset+1)*size);
					return `[${components.join(', ')}]`;
				}
				//calculated access should slice array
				else {
					return `${left}.slice(${right}*${size}, (${right} + 1) * ${size})`;
				}
			}
		...
		if (!this.primitives[leftType] || !this.primitives[rightType]) {
			...
			//num * vec → [vec[0] * n, vec[n] * n]
			//vec * num → [n * vec[0], n * vec[n]]
			//vec * vec → [vec[0] * vec[0], vec[n] * vec[n]]
			//NOTE: no matrices allowed
			if ((!/mat/.test(leftType) && !/mat/.test(rightType)) && this.complexity(node.children[0]) + this.complexity(node.children[1]) < this.maxComplexity) {
				...
				for (var i = 0; i < l; i++) {
					...
					if (operation == null) {
						operation = `${leftOp} ${operator} ${rightOp}`;
					}
			...
			//include operations holder
			this.addInclude(outType, this.operatorNames[operator]);
