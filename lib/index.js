...
GLSL.prototype.preprocess = true;
...
/**
 * Apply preprocessor of a current state for descriptor
 */
GLSL.prototype.preprocessDescriptor = function (desc) {
	var str = desc + '';
	var self = this;
	//replace descriptor str while there is no more calls
	for (var prop in this.define) {
		//fn replacement
		if (/\)/.test(prop)) {
			var name = /[a-z0-9_]+/i.exec(prop)[0];
			var re = new RegExp(name + '(\\([^\\)]*\\))', 'g');
			str = str.replace(re, function (match, args, idx, str) {
				var fnArgs = args[1].slice(1, -1).trim();
				...
				var res = self.define[prop].apply(self, fnArgs);
				return res;
			});
		}
		//simple replacement
			...
			str = str.replace(re, function (match, idx, str) {
				return self.define[prop].call(self);
			});
		}
	}
	console.log('prepro res', str)

	return Descriptor(str, desc);
}
...
GLSL.prototype.transforms = {
	...
	function: function (node) {
		...
		if (this.functions[name]) name = `${name}_${argTypesSfx}`;
	...
	decl: function (node) {
		...
		if (node.token.data === 'struct') {
			...
			result = this.process(decllist, this.replaceAttribute);
		}
		else if (node.token.data === 'varying') {
			result = this.process(decllist, this.replaceVarying);
		}
		else if (node.token.data === 'uniform') {
			result = this.process(decllist, this.replaceUniform);
		}
	...
	preprocessor: function (node) {
		...
		if (/^define/.test(macroName)) {
			...
			//sort out macro args
			if (macroArgs) {
				fnArgs = macroArgs.slice(1, -1);

				if (fnArgs.length) {
					fnArgs = fnArgs.split(/\s*,\s*/).map(function (name, pos) {
						//escape all strings
						var strings = [];
						...
						//replace each stringified name notion
			...
			var fn = new Function(`
				${fnArgs.join('\n ')}
				console.group("${varName}", arguments, \`'${rest}'\`)
				var res = this.preprocessDescriptor('${rest}');
				console.log(res);
				console.groupEnd();
				return res;
			`);
			this.define[varName] = fn;
		}
	...
	ident: function (node) {
		...
		//apply preprocessor
		if (this.preprocess) {
			if (this.define.hasOwnProperty(str)) {
				str = this.define[str].call(this);
			}
		}
	...
	assign: function (node) {
		...
		//a *= b.x
		if (!isSwizzle && this.types[rightType].length == 1) {
	...
	call: function (node) {
		...
		if (node.children[0].data === '.') {
			...
			return Descriptor(`${holderName}.${methodName}`, {
		...
		//else treat as function/constructor call
			...
			if (this.preprocess) {
				if (this.define.hasOwnProperty(callName + '()')) {
					var res = this.define[callName + '()'].apply(this, argValues);
			...
			//someFn()
				...
				var res = Descriptor(`${callName}(${argValues.join(', ')})`, {
					...
					complexity: argValues.reduce(function (prev, curr) {
						return curr.complexity+prev;
					}, callName.complexity||0),
					optimize: optimize
				});
