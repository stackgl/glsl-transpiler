...
GLSL.prototype.process = function stringify (node, arg) {
	...
	if (node instanceof String) this.cache(node, node);
...
GLSL.prototype.transforms = {
	...
	assign: function (node) {
		...
		//a *= b.x
		if (!isSwizzle && this.primitives[rightType]) {
		...
		if (isSwizzle) {
			...
			if (positions.length > 1) {
				...
				return Descriptor(
					`${target} = [${ids.join(', ')}].map(function (idx, i) { return idx == null ? ${target}[i] : this[idx]; }, ${right})`, {
						type: rightType,
