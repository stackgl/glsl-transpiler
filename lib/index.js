	...
	for (var prop in this.macro) {
		...
		if (/\)/.test(prop)) {
			...
			str = str.replace(re, function (match, args, idx, str) {
				...
				var res = self.macro[prop].apply(self, fnArgs);
				return res;
			});
		}
		//simple replacement
			...
			str = str.replace(re, function (match, idx, str) {
				return self.macro[prop].call(self);
			});
		}
	}
...
GLSL.prototype.transforms = {
	...
	preprocessor: function (node) {
		...
		if (/^define/.test(directive)) {
			var restIdx = str.indexOf(macroName);
			var rest = str.slice(restIdx + macroName.length + (directiveParams ? directiveParams.length : 0)).trim();
			var fnArgs = [];

			//sort out macro args
			if (directiveParams) {
				fnArgs = directiveParams.slice(1, -1);
			...
			var fn = new Function(`
				...
				return result;
			`);
			this.macro[macroName] = fn;
		}

		if (/^undef/.test(directive)) {
			var macroNameOnly = /[a-z0-9_]+/i.exec(macroName)[0];
			delete this.macro[macroNameOnly];
			delete this.macro[macroNameOnly+'()'];
		}
