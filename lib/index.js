...
GLSL.prototype.process = function stringify (node, arg) {
	...
	if (node == null ||
		...
		typeof node === 'string' ||
		typeof node === 'boolean' ||
		node instanceof String) return this.cache(node, Descriptor(node));
	...
	if (t === undefined) {
		...
		return this.cache(node, Descriptor(null));
	}
	...
	if (typeof t !== 'function') {
		return this.cache(node, Descriptor(t));
	}
	...
	var result = this.cache(node, Descriptor(t.call(this, node, arg)));
...
GLSL.prototype.transforms = {
	...
	stmt: function (node) {
		...
		result += node.children.map(this.process, this).join('');
		if (result) result += ';';
		...
		this.structures[structName] = function (args) {
			...
			var fields = argsList.map(function (argName, i) {
				if (args[i]) {
					...
					var initValue = this.createType(argTypes[i], args[i]);
				}
	...
	suffix: function (node) {
		return this.process(node.children[0]) + node.data;
	},
	...
	ident: function (node) {
		//look up for defined items, if any
		if (this.define[node.data]) {
			var val = this.define[node.data];
			return val;
		}
	...
	binary: function (node) {
		...
		if (node.data === '[') {
			...
			if (leftNode.type === 'binary') {
				...
				var matType = this.getType(matNode);
				...
				if (!isNaN(idx) && !isNaN(offset)) {
					return this.getComponent(matNode, vecSize*idx + offset);
				}

				//if calc - do slice
				else {
					return `${this.process(matNode)}[${outerRight} * ${vecSize} + ${right}]`;
				}
			}

			//matrix single access a[0] → vec
			if (/mat/.test(leftType)) {
				return this.getMatrixComponent(leftNode, right);
			}
		...
		var str = this.renderOperation(leftNode, rightNode, operator);
		this.setComponents(node, str.components);
	...
	unary: function (node) {
		...
		if (node.data === '+') {
			...
			if (node.children[0].type === 'unary') {
				return node.data + this.process(node.children[0]);
			}
			...
			//+x
			return this.process(node.children[0]);
		}
		return node.data + this.process(node.children[0]);
	},
	...
	call: function (node) {
		...
		//else treat as function/constructor call
			...
			//macro()
			if (this.define.hasOwnProperty(callName + '()')) {
				return this.define[callName + '()'];
			}
			...
			//struct()
				...
				result += this.structures[callName].call(this, args);
			}
			...
			//someFn()
				...
				if (this.stdlib[callName]) {
					...
					//TODO: implement propagating types (save per-comp values)
					// var type = this.getType(node);
					// var argType = this.getType(args[0]);

					// //stdlib(type)
					// if (this.types[argType].length > 1) {

					// }
				}

				return `${callName}(${args.map(this.process, this).join(', ')})`;
			}
		}
...
GLSL.prototype.unswizzle = function (node) {
	...
	var complexity = args.length * this.complexity(identNode);
	...
	//a.yz → [1, 2].map(function(x) { return this[x]; }, a)
		...
		var result = new String(`[${positions.join(', ')}].map(function (x, i) { return this[x]}, ${ident})`);
	}
...
GLSL.prototype.complexity = function (node) {
	if (!this.optimize) return this.maxComplexity;

	var self = this;

	//simple objects get 0 complexity, as we guess they are ready
	if (typeof node !== 'object') return 0;

	//string objects get special complexity
...
GLSL.prototype.getComponent = function (node, n) {
	...
	//if index is not a number - return access component
	if (isNaN(idx)) {
		return `${this.process(node)}[${idx}]`;
	}

	n = idx;

	//if stringify found a shorter component values - use them
	//FIXME: node can have components of it’s prototype (ident). We can try to update prototype’s values for each `setComponents` call so that optimization there will be better (constants hoist etc)
	if (this.optimize && node.hasOwnProperty('_components') && node._components[n] != null) {
	...
	//NOTE: known types always contain inner components.
	//FIXME: a.x is also primitive but not simply insertable. Filter bad cases here.
	if (this.primitives[nodeType] && node.type !== 'binary') {
		return this.process(node);
	}
	//basic way to access components in js: a[n]
	else {
		return `${this.process(node)}[${n}]`;
	}
}
...
GLSL.prototype.getMatrixComponent = function (node, n) {
	...
	if (!isNaN(offset) && this.complexity(node)*size < this.maxComplexity) {
		var components = this.getComponents(node).slice(offset*size, (offset+1)*size);
		return `[${components.join(', ')}]`;
	}
	//calculated access should slice array
	else {
		return `${this.process(node)}.slice(${n}*${size}, (${n} + 1) * ${size})`;
	}
}
...
GLSL.prototype.getComponents = function (node) {
	...
	for (var i = 0, l = Math.max(this.types[nodeType].length, node._components && node._components.length || 0), c; i < l; i++) {
...
GLSL.prototype.setComponents = function (node, components) {
	...
	node._components = [];
	for (var i = 0; i < Math.max(components.length, len); i++) {
...
GLSL.prototype.createType = function (typeName, node, args) {
	if (this.structures[typeName]) return this.structures[typeName].call(this, args);
 ...
 *
 * @param {string} str Difficult case string result (default string result)
 * @param {Array} components List of nodes - components of the constructor
 * @param {*} includes List of includes if complexity is high
 */
GLSL.prototype.createTypeResult = function (str, compNodes, typeName, includes) {
	//rendered components
	var components = [];
	...
	for (var i = 0; i < compNodes.length; i++) {
		...
		//pass-through simple components
		if (curr instanceof String) {
			comp = curr;
		}
		else {
			//try to fetch component from the inner node - propagate components
	...
	if (this.complexity(compNodes) >= this.maxComplexity) {
		//the only component means that that is simple type, not vec/mat, so avoid mapping
		...
		// if (/\[/.test(str)) console.warn(str + ' is repeating type result, pas bon')

		this.addInclude(includes);

		var result = new String(str);
	}
	//low complexity allows for just joining components
	else {
		if (components.length > 1) {
			var result = new String(`[${components.join(', ')}]`);
		}
		else {
			var result = new String(components[0]);
		}
	}
