		...
		if (node.data === '[') {
			...
			if (leftNode.type === 'binary') {
				...
				if (!isNaN(idx) && !isNaN(offset)) {
					return Descriptor(this.getComponent(matNode, vecSize*idx + offset), {
						type: 'float',
						complexity: matNode.complexity + right.complexity + 1
					})
				}

				//if calc - do slice
				else {
					return Descriptor(`${this.process(matNode)}[${outerRight} * ${vecSize} + ${right}]`, {
						type: 'float',
