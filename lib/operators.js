...
function processOperation (leftDesc, rightDesc, operator) {
	...
	if (this.primitives[leftType] || this.primitives[rightType]) {
		...
		var operands = [];
		for (var i = 0; i < l; i++) {
			if (this.primitives[rightType]) {
				var rightOp = rightDesc, leftOp = leftDesc.components[i];
			}
			else {
				var rightOp = rightDesc.components[i], leftOp = leftDesc;
			}
			operands.push(calculate(leftOp, rightOp, operator));
		}
		var calcStr = this.primitives[rightType] ? calculate(Descriptor('x'), scalar, operator) :  calculate(scalar, Descriptor('x'), operator);
		var res = Descriptor(
			`${vec}.map(function (x, i) {return ${calcStr};})`, {
			components: operands,
			type: outType,
