...
function bool (node) {
	if (node == null) return Descriptor(false, {type: 'bool'});
	...
	//bool?
	if (result == 'true' || result === true) return Descriptor(true, {type: 'bool'});
	if (result == 'false' || result === false) return Descriptor(false, {type: 'bool'});
	...
	if (isNaN(num)) {
		return Descriptor('!!' + result, {type: 'bool'});
	}

	//cast number to bool
	return Descriptor(!!num, {type: 'bool'});
}
bool.type = 'bool';
...
function int (node) {
	if (node == null) return Descriptor(0, {type: 'int'});

	if (typeof node !== 'object') return Descriptor(+node|0, {type: 'int'});
	...
	//bool?
	if (result == 'true' || result === true) return Descriptor(1, {type: 'int'});
	if (result == 'false' || result === false) return Descriptor(0, {type: 'int'});

	var num = parseFloat(result);

	//it was number
	if (!isNaN(num)) {
		return Descriptor(+num|0, {type: 'int'});
	}
	//it was string
	else {
		result += '|0';
	...
	return Descriptor(result, {type: 'int'});
}
int.type = 'int';
