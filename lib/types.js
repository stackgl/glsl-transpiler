...
function createVec2(type, vecType) {
	...
	function vec2(x, y) {
		...
		if (this.types[x.type].length > 2) {
			return Descriptor(`${x}.subarray(0, 2)${map}`, {
		...
		if (x === y) {
			return Descriptor(`new Float32Array([0, 0]).fill(${x})${map}`, {
				complexity: x.complexity + 2,
		...
		return Descriptor(`new Float32Array([${[x, y].join(', ')}])${map}`, {
			components: [x, y].map(this.types[type], this),
			type: vecType,
...
function createVec3(type, vecType) {
	...
	function vec3(x, y, z) {
		...
		if (this.types[x.type].length > 3) {
			return Descriptor(`${x}.subarray(0, 3)${map}`, {
		...
		//vec3(vec2, _) → vec2.concat(_)
		if (this.types[x.type].length === 2) {
			return Descriptor(`new Float32Array([...${x}, ${this.types.float.call(this, y)}])${map}`, {
		...
		//vec3(float, vecN) → [float, ...vecN.slice(0,2)];
		if (this.types[y.type].length > 1) {
			return Descriptor(`new Float32Array([${x}, ...${this.types.vec2.call(this, y, z)}])${map}`, {
				components: [x, ...y.components.slice(0, 2)].map(this.types[type], this),
				type: vecType,
		...
		return Descriptor(`new Float32Array([${[x, y, z].join(', ')}])${map}`, {
...
function createVec4(type, vecType) {
	...
	function vec4(x, y, z, w) {
		...
		//vec4(vecN) → vecN.subarray(0, 4)
		if (this.types[x.type].length > 4) {
			return Descriptor(`${x}.subarray(0, 4)${map}`, {
		...
		if (this.types[x.type].length === 3) {
			return Descriptor(`new Float32Array([...${x}, ${this.types.float.call(this, y)}])${map}`, {
				components: [...x.components, y.components[0]].map(this.types[type], this),
				type: vecType,
				include: include
			});
		}
		...
		if (this.types[x.type].length === 2) {
			//vec4(vec2, vecN)
			if (this.types[y.type].length > 1) {
				return Descriptor(`new Float32Array([...${x}, ${this.types.vec2.call(this, y)}])${map}`, {
			...
			var res = Descriptor(
				`new Float32Array([...${x}, ...${this.types.vec2.call(this, y, z)}])${map}`, {
		...
		if (this.types[y.type].length === 2) {
			return Descriptor(`new Float32Array([${x}, ...${this.types.vec2.call(this, y)}, ${this.types.float.call(this, z)}])${map}`, {
		...
		//vec4(float, vecN)
		if (this.types[y.type].length > 2) {
			return Descriptor(`new Float32Array([${x}, ...${this.types.vec3.call(this, y, z, w)}])${map}`, {
		...
		if (this.types[z.type].length > 1) {
			return Descriptor(`new Float32Array([${x}, ${y}, ...${this.types.vec2.call(this, z)}])${map}`, {
		...
		return Descriptor(`new Float32Array([${[x, y, z, w].join(', ')}])${map}`, {
...
function mat2(v0, v1) {
	...
	return Descriptor(`new Float32Array([...${this.types.vec2.call(this, v0)}, ...${this.types.vec2.call(this, v1)}])`, {
...
function mat3(v0, v1, v2) {
	...
	//mat3(mat2)
	if (v0.type === 'mat2') {
		return Descriptor(`new Float32Array([0,1,null, 2,3,null, null,null,-1].map(function (i) {return i == null ? 0 : i < 0 ? -i : this[i]}, ${v0}))`, {
			components: [
	...
	if (v0.type === 'mat4') {
		...
		return Descriptor(`new Float32Array(${v0}.filter((x, i) => i % 4 !== 3 && i < 12;))`, {
	...
	var res = Descriptor(`new Float32Array([...${this.types.vec3.call(this, v0)}, ...${this.types.vec3.call(this, v1)}, ...${this.types.vec3.call(this, v2)}])`, {
		components: comps.map(float, this),
		type: 'mat3',
		complexity: cmpl(comps)
	});
...
function mat4(v0, v1, v2, v3) {
	...
	if (v0.type === 'mat2') {
		return Descriptor(
			`new Float32Array([0,1,null,null, 2,3,null,null, null,null,-1,null, null,null,null,-1].map(function (i) {return i == null ? 0 : i < 0 ? -i : this[i]}, ${v0}))`, {
			components: [
	...
	if (v0.type === 'mat3') {
		var components = v0.components;
		return Descriptor(
			`new Float32Array([0,1,2,null,3,4,5,null,6,7,8,null,null,null,null,-1].map(function (i) {return i == null ? 0 : i < 0 ? -i : this[i]}, ${v0}))`, {
	...
	return Descriptor(`new Float32Array([...${this.types.vec4.call(this, v0)}, ...${this.types.vec4.call(this, v1)}, ...${this.types.vec4.call(this, v2)}, ...${this.types.vec4.call(this, v3)}])`, {
		components: comps.map(float, this),
		type: 'mat4',
		complexity: cmpl(comps)
	});
