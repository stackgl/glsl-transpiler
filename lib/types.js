...
function createVec2 (type) {
	...
	function vec2 (x, y) {
		...
		if (this.types[xType].length === 2) {
			return this.createTypeResult(this.process(x), [x, x], type);
		}

		//vec2(vec3) → vec3.slice(0, 2)
		if (this.types[xType].length > 2) {
			return this.createTypeResult(`${this.process(x)}.slice(0, 2)`, [x, x], type);
		};

		//vec2(complex, complex) → [0, 0].fill(complex)
		if (x === y && this.complexity(x) > 5) {
			return this.createTypeResult(`[0, 0].fill(${this.process(x)})`, [x, y], type);
		}

		//vec2(simple, simple) → [simple, simple]
		return this.createTypeResult(`[${[x,y].map(this.process, this).join(', ')}]`, [x, y], type);
	}
	return vec2;
};
...
function createVec3 (type) {
	...
	function vec3 (x, y, z) {
		...
		if (this.types[xType].length === 3) {
			return this.createTypeResult(this.process(x), [x, x, x], type);
		}

		//vec3(vecN) → vecN.slice(0, 3)
		if (this.types[xType].length > 3) {
			return this.createTypeResult(`${this.process(x)}.slice(0, 3)`, [x, x, x], type);
		}

		//vec3(vec2, *) → vec2.concat(*)
		if (this.types[xType].length === 2) {
			return this.createTypeResult(`${this.process(x)}.concat(${this.types.float.call(this, y)})`, [x, x, y], type);
		}

		//vec3(float, vecN) → [float].concat(vecN.slice(0,2));
		if (this.types[yType].length > 1) {
			return this.createTypeResult(`[${this.process(x)}].concat(${this.types.vec2.call(this, y, z)})`, [x, y, y], type);
		}

		return this.createTypeResult(`[${[x,y,z].map(this.process, this).join(', ')}]`, [x, y, z], type);
	}
	return vec3;
};
...
function createVec4 (type) {
	...
	function vec4 (x, y, z, w) {
		...
		//vec4(matN)
		if (/mat/.test(xType)) {
			return this.createTypeResult(this.process(x), this.getComponents(x), type);
		}

		//vec4(vecN) → vecN.slice(0, 4)
		if (this.types[xType].length > 4) {
			return this.createTypeResult(`${this.process(x)}.slice(0, 4)`, [x, x, x, x], type);
		}

		//vec4(vec4) → vec4
		if (this.types[xType].length === 4) {
			return this.createTypeResult(this.process(x), [x, x, x, x], type);
		}

		//vec4(vec3, *) → vec3.concat(*)
		if (this.types[xType].length === 3) {
			return this.createTypeResult(`${this.process(x)}.concat(${this.types.float.call(this, y)})`, [x, x, x, y], type);
		}
		...
		if (this.types[xType].length === 2) {
			//vec4(vec2, vecN)
			if (this.types[yType].length > 1) {
				return this.createTypeResult(`${this.process(x)}.concat(${this.types.vec2.call(this, y)})`, [x, x, y, y], type);
			}

			//vec4(vec2, float, float)
			return this.createTypeResult(
				`${this.process(x)}.concat(${this.types.vec2.call(this, y, z)})`, [x, x, y, z], type);
		}

		//vec4(float, vec2, *)
		if (this.types[yType].length === 2) {
			return this.createTypeResult(`[${this.process(x)}].concat(${this.types.vec2.call(this, y)}, ${this.types.float.call(this, z)})`, [x, y, y, z], type);
		}

		//vec4(float, vecN)
		if (this.types[yType].length > 2) {
			return this.createTypeResult(`[${this.process(x)}].concat(${this.types.vec3.call(this, y, z, w)})`, [x, y, y, y], type);
		}

		//vec4(float, float, vecN)
		if (this.types[zType].length > 1) {
			return this.createTypeResult(`[${this.process(x)}].concat(${this.process(y)}, ${this.types.vec2.call(this, z)})`, [x, y, z, z], type);
		}

		return this.createTypeResult(`[${[x,y,z,w].map(this.process, this).join(', ')}]`, [x, y, z, w], type);
	}
	return vec4;
}
...
function mat2 (v0, v1) {
	...
	if (arguments.length >= 4) {
		...
		var y1 = arguments[3];
		return this.createTypeResult(
			`[${[x0, y0, x1, y1].map(this.process, this).join(', ')}]`,
			[x0, y0, x1, y1], 'vec2');
	};
	...
	//mat2(mat2)
	if (v0Type === 'mat2') {
		return this.createTypeResult(this.process(v0), this.getComponents(v0), 'vec2');
	}
...
function mat3 (v0, v1, v2) {
	...
	//mat3(mat3)
	if (v0Type === 'mat3') {
		return this.createTypeResult(this.process(v0), this.getComponents(v0), 'vec3');
	}

	//mat3(mat4)
	if (v0Type === 'mat4') {
		var components = this.getComponents(v0);
		return this.createTypeResult(`${this.process(v0)}.filter(function (x, i) { return i % 4 !== 3 && i < 12; })`, components.slice(0, 3).concat(components.slice(4, 7), components.slice(8, 11)), 'vec3');
	}
