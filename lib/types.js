...
function bool (node) {
	...
	if (this.types[type] && this.types[type].length > 1) {
		return this.getComponent(node, 0);
	}
...
function mat2 (x0, y0, x1, y1) {
	return this.createTypeResult(`[${[x0, y0, x1, y1].map(this.stringify, this).join(', ')}]`, [x0, y0, x1, y1]);
}

function mat3 (x0, y0, z0, x1, y1, z1) {
	return this.createTypeResult(`[${[x0, y0, z0, x1, y1, z1].map(this.stringify, this).join(', ')}]`, [x0, y0, z0, x1, y1, z1]);
}

function mat4 (x0, y0, z0, w0, x1, y1, z1, w0) {
