...
function bool (node) {
	if (node == null) return Descriptor(false);
	...
	//bool?
	if (result == 'true' || result === true) return Descriptor(true);
	if (result == 'false' || result === false) return Descriptor(false);
...
function int (node) {
	if (node == null) return Descriptor(0);

	if (typeof node !== 'object') return Descriptor(+node|0);
	...
	//bool?
	if (result == 'true' || result === true) return Descriptor(1);
	if (result == 'false' || result === false) return Descriptor(0);

	var num = parseFloat(result);

	//it was number
	if (!isNaN(num)) {
		return Descriptor(+num|0);
	}
	//it was string
	else {
		result += '|0';
...
function float (node) {
	...
	//bool?
	if (result == 'true' || result === true) return Descriptor(1.0);
	if (result == 'false' || result === false) return Descriptor(0.0);
...
function createVec3 (type) {
	...
	function vec3 (x, y, z) {
		...
		if (this.types[x.type].length > 3) {
			return Descriptor(`${x}.slice(0, 3).map(${type})`, {
		...
		if (this.types[x.type].length === 2) {
			return Descriptor(`${x}.concat(${this.types.float.call(this, y)}).map(${type})`, {
		...
		if (this.types[y.type].length > 1) {
			return Descriptor(`[${x}].concat(${this.types.vec2.call(this, y, z)}).map(${type})`, {
				components: [x].concat(y.components.slice(0, 2)).map(this.types[type], this),
				type: 'vec3'
			});
		}

		return Descriptor(`[${[x,y,z].join(', ')}].map(${type})`, {
...
function createVec4 (type) {
	...
	function vec4 (x, y, z, w) {
		...
		if (this.types[x.type].length === 3) {
			return Descriptor(`${x}.concat(${this.types.float.call(this, y)})`, {
				components: x.components.concat(y.components[0]).map(this.types[type], this),
				type: 'vec4'
			});
		}
		...
		if (this.types[x.type].length === 2) {
			//vec4(vec2, vecN)
			if (this.types[y.type].length > 1) {
				return Descriptor(`${x}.concat(${this.types.vec2.call(this, y)})`, {
			...
			return Descriptor(
				`${x}.concat(${this.types.vec2.call(this, y, z)})`, {
		...
		if (this.types[y.type].length === 2) {
			return Descriptor(`[${x}].concat(${this.types.vec2.call(this, y)}, ${this.types.float.call(this, z)})`, {
		...
		//vec4(float, vecN)
		if (this.types[y.type].length > 2) {
			return Descriptor(`[${x}].concat(${this.types.vec3.call(this, y, z, w)})`, {
...
function mat3 (v0, v1, v2) {
	...
	if (arguments.length >= 9) {
		...
		return this.createTypeResult(
			`[${[x0, y0, z0, x1, y1, z1, x2, y2, z2].map(this.process, this).join(', ')}]`,
			[x0, y0, z0, x1, y1, z1, x2, y2, z2], 'mat3');
	};

	//ensure at least identity matrix
	if (v0 == null) v0 = 1;

	var v0Type = this.process(v0).type;
	var v1Type = this.process(v1).type;
	var v2Type = this.process(v2).type;

	//mat3(float) → identity matrix
	if (this.types[v0Type].length === 1) {
		var res = this.createTypeResult(
			`mat3(${this.process(v0)})`,
			[
				v0, 0, 0,
				0, v0, 0,
				0, 0, v0
	...
	//mat3(mat2)
	if (v0Type === 'mat2') {
		return this.createTypeResult(`[0,1,null, 2,3,null, null,null,-1].map(function (i) {return i == null ? 0 : i < 0 ? -i : this[i]}, ${this.process(v0)})`,
			[
	...
	//mat3(mat3)
	if (v0Type === 'mat3') {
		return this.createTypeResult(this.process(v0), this.process(v0).components, 'mat3');
	}

	//mat3(mat4)
	if (v0Type === 'mat4') {
		var components = this.process(v0).components;
		return this.createTypeResult(`${this.process(v0)}.filter(function (x, i) { return i % 4 !== 3 && i < 12; })`, components.slice(0, 3).concat(components.slice(4, 7), components.slice(8, 11)), 'mat3');
	}

	//mat(vec, vec, vec)
	return this.createTypeResult(`${this.types.vec3.call(this, v0)}.concat(${this.types.vec3.call(this, v1)}, ${this.types.vec3.call(this, v2)})`, [v0, v0, v0, v1, v1, v1, v2, v2, v2], 'mat3');
}
mat3.type = 'vec3';
...
function mat4 (v0, v1, v2, v3) {
	...
	if (arguments.length >= 16) {
		...
		return this.createTypeResult(
			`[${[x0, y0, z0, w0, x1, y1, z1, w1, x2, y2, z2, w2, x3, y3, z3, w3].map(this.process, this).join(', ')}]`,
			[x0, y0, z0, w0, x1, y1, z1, w1, x2, y2, z2, w2, x3, y3, z3, w3], 'mat4');
	};
	...
	var v1Type = this.process(v1).type;
	var v2Type = this.process(v2).type;
	var v3Type = this.process(v3).type;

	//mat(float) → identity matrix
	if (this.types[v0Type].length === 1) {
		var res = this.createTypeResult(
			`mat4(${this.process(v0)})`,
			[
				v0, 0, 0, 0,
				0, v0, 0, 0,
				0, 0, v0, 0,
	...
	if (v0Type === 'mat2') {
		return this.createTypeResult(
			`[0,1,null,null, 2,3,null,null, null,null,-1,null, null,null,null,-1].map(function (i) {return i == null ? 0 : i < 0 ? -i : this[i]}, ${this.process(v0)})`,
			[
	...
	if (v0Type === 'mat3') {
		var components = this.process(v0).components;
		return this.createTypeResult(
			`[0,1,2,null,3,4,5,null,6,7,8,null,null,null,null,-1].map(function (i) {return i == null ? 0 : i < 0 ? -i : this[i]}, ${this.process(v0)})`,
			components.slice(0, 3).concat(0, components.slice(3, 6), 0, components.slice(6, 9), 0, 0, 0, 0, 1), 'mat4');
	}

	//mat(vec, vec, vec, vec)
	return this.createTypeResult(`${this.types.vec4.call(this, v0)}.concat(${this.types.vec4.call(this, v1)}, ${this.types.vec4.call(this, v2)}, ${this.types.vec4.call(this, v3)})`, [v0, v0, v0, v0, v1, v1, v1, v1, v2, v2, v2, v2, v3, v3, v3, v3], 'mat4');
}
mat4.type = 'vec4';
