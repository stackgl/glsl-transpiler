...
function float (node) {
	...
	if (typeof node === 'object') {
		...
		if (this.types[type] && this.types[type].length > 1) {
			result = this.getComponent(node, 0);
			// return this.createTypeResult(this.getComponent(node, 0), this.getComponents(node));
		}
		else {
			result = this.stringify(node);
		}
...
function createVec4 (type) {
	return function vec4 (x, y, z, w) {
		...
		//vec4(matN)
		if (/mat/.test(xType)) {
			var v0 = this.createCallNode('vec2', this.getComponent(x, 0).map(this.stringify, this).slice(0, 2));
			var v1 = this.createCallNode('vec2', this.getComponent(x, 1).map(this.stringify, this).slice(0, 2));

			return this.types.vec4.call(this, v0, v1);
		}
...
function mat2 (v0, v1) {
	...
	if (arguments.length >= 4) {
		...
		var y1 = arguments[3];
		return this.createTypeResult(
			`[${this.types.vec2.call(this, x0, y0)}, ${this.types.vec2.call(this, x1, y1)}]`,
			[
				this.createCallNode('vec2', [x0, y0].map(this.stringify, this)),
				this.createCallNode('vec2', [x1, y1].map(this.stringify, this))
			], 'vec2');
	};
	...
	if (this.types[v0Type].length === 1) {
		var res = this.createTypeResult(
			`[${this.types.vec2.call(this, v0, 0)}, ${this.types.vec2.call(this, 0, v0)}]`,
			[
				this.createCallNode('vec2', [v0, 0].map(this.stringify, this)),
				this.createCallNode('vec2', [0, v0].map(this.stringify, this))
			], 'vec2');
		return res;
	}

	//mat(vec, vec)
	return this.createTypeResult(`[${[v0, v1].map(this.stringify, this).join(', ')}]`, [v0, v1], 'vec2');
}
...
function mat3 (v0, v1, v2) {
	...
	if (arguments.length >= 9) {
		...
		var z2 = arguments[8];
		return this.createTypeResult(
			`[${this.types.vec3.call(this, x0, y0, z0)}, ${this.types.vec3.call(this, x1, y1, z1)}, ${this.types.vec3.call(this, x2, y2, z2)}]`,
			[
				this.createCallNode('vec3', [x0, y0, z0].map(this.stringify, this)),
				this.createCallNode('vec3', [x1, y1, z1].map(this.stringify, this)),
				this.createCallNode('vec3', [x2, y2, z2].map(this.stringify, this))
			], 'vec3');
	};
	...
	if (this.types[v0Type].length === 1) {
		var res = this.createTypeResult(
			`[${this.types.vec3.call(this, v0, 0, 0)}, ${this.types.vec3.call(this, 0, v0, 0)}, ${this.types.vec3.call(this, 0, 0, v0)}]`,
			[
				this.createCallNode('vec3', [v0, 0, 0].map(this.stringify, this)),
				this.createCallNode('vec3', [0, v0, 0].map(this.stringify, this)),
				this.createCallNode('vec3', [0, 0, v0].map(this.stringify, this))
			], 'vec3');
		return res;
	}
	...
	if (v0Type === 'mat2') {
		var v = v0;
		v0 = this.createCallNode('vec3', this.getComponent(v, 0).map(this.stringify, this).concat(0));
		v1 = this.createCallNode('vec3', this.getComponent(v, 1).map(this.stringify, this).concat(0));
		v2 = this.createCallNode('vec3', [0, 0, 1]);
	}

	//mat3(mat3)
	if (v0Type === 'mat3') {
		var v = v0;
		v0 = this.createCallNode('vec3', this.getComponent(v, 0).map(this.stringify, this));
		v1 = this.createCallNode('vec3', this.getComponent(v, 1).map(this.stringify, this));
		v2 = this.createCallNode('vec3', this.getComponent(v, 2).map(this.stringify, this));
	}

	//mat3(mat4)
	if (v0Type === 'mat4') {
		var v = v0;
		v0 = this.createCallNode('vec3', this.getComponent(v, 0).slice(0, 3).map(this.stringify, this));
		v1 = this.createCallNode('vec3', this.getComponent(v, 1).slice(0, 3).map(this.stringify, this));
		v2 = this.createCallNode('vec3', this.getComponent(v, 2).slice(0, 3).map(this.stringify, this));
	}

	//mat(vec, vec, vec)
	return this.createTypeResult(`[${[v0, v1, v2].map(this.stringify, this).join(', ')}]`, [v0, v1, v2], 'vec3');
}
...
function mat4 (v0, v1, v2, v3) {
	...
	if (arguments.length >= 16) {
		...
		return this.createTypeResult(
			`[${this.types.vec4.call(this, x0, y0, z0, w0)}, ${this.types.vec4.call(this, x1, y1, z1, w1)}, ${this.types.vec4.call(this, x2, y2, z2, w2)}, ${this.types.vec4.call(this, x3, y3, z3, w3)}]`,
			[
				this.createCallNode('vec4', [x0, y0, z0, w0].map(this.stringify, this)),
				this.createCallNode('vec4', [x1, y1, z1, w1].map(this.stringify, this)),
				this.createCallNode('vec4', [x2, y2, z2, w2].map(this.stringify, this)),
				this.createCallNode('vec4', [x3, y3, z3, w3].map(this.stringify, this))
			], 'vec4');
	};
	...
	if (this.types[v0Type].length === 1) {
		var res = this.createTypeResult(
			`[${this.types.vec4.call(this, v0, 0, 0, 0)}, ${this.types.vec4.call(this, 0, v0, 0, 0)}, ${this.types.vec4.call(this, 0, 0, v0, 0)}, ${this.types.vec4.call(this, 0, 0, 0, v0)}]`,
			[
				this.createCallNode('vec4', [v0, 0, 0, 0].map(this.stringify, this)),
				this.createCallNode('vec4', [0, v0, 0, 0].map(this.stringify, this)),
				this.createCallNode('vec4', [0, 0, v0, 0].map(this.stringify, this)),
				this.createCallNode('vec4', [0, 0, 0, v0].map(this.stringify, this))
			], 'vec4');
		return res;
	}
	...
	if (v0Type === 'mat2') {
		var v = v0;
		v0 = this.createCallNode('vec4', this.getComponent(v, 0).map(this.stringify, this).concat(0, 0));
		v1 = this.createCallNode('vec4', this.getComponent(v, 1).map(this.stringify, this).concat(0, 0));
		v2 = this.createCallNode('vec4', this.getComponent(v, 2).map(this.stringify, this).concat(0, 0));
		v3 = this.createCallNode('vec4', [0, 0, 0, 1]);
	}

	//mat4(mat3)
	if (v0Type === 'mat3') {
		var v = v0;
		v0 = this.createCallNode('vec4', this.getComponent(v, 0).map(this.stringify, this).concat(0));
		v1 = this.createCallNode('vec4', this.getComponent(v, 1).map(this.stringify, this).concat(0));
		v2 = this.createCallNode('vec4', this.getComponent(v, 2).map(this.stringify, this).concat(0));
		v3 = this.createCallNode('vec4', [0, 0, 0, 1]);
	}

	//mat(vec, vec, vec, vec)
	return this.createTypeResult(`[${[v0, v1, v2, v3].map(this.stringify, this).join(', ')}]`, [v0, v1, v2, v3], 'vec4');
}
