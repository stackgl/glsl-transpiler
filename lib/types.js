...
vec.prototype.multiply = function (arg) {
	var len = this.length();
	var vec = this.constructor['vec' + len]();
	...
	if (arg instanceof mat) {
		...
		vec[0] = this[0] * arg[0];
		vec[1] = this[1] * arg[1];
		if (len > 2) vec[2] = this[2] * arg[2];
		if (len > 3) vec[3] = this[3] * arg[3];
	}
	else {
		arg = float(arg);
		vec[0] = this[0] * arg;
		vec[1] = this[1] * arg;
		if (len > 2) vec[2] = this[2] * arg;
		if (len > 3) vec[3] = this[3] * arg;
	}
...
vec.prototype.subtract = function (arg) {
	...
	if (arg instanceof mat) {
		...
		vec[0] = type(this[0] - arg[0]);
		vec[1] = type(this[1] - arg[1]);
		if (len > 2) vec[2] = type(this[2] - arg[2]);
		if (len > 3) vec[3] = type(this[3] - arg[3]);
	}
	else {
		arg = float(arg);
		vec[0] = type(this[0] - arg);
		vec[1] = type(this[1] - arg);
		if (len > 2) vec[2] = type(this[2] - arg);
		if (len > 3) vec[3] = type(this[3] - arg);
	}
